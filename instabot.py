...
# Only while acessing a private account that you follow -- bot.login(user="Your_username",passwd="Your_password") #Use this code to log-in to your account.
...
def downloadPost():
    ...
    for index, post in enumerate(posts, 1):
        if count < 5:
            bot.download_post(post, target=f"{profile.username}_{index}") #setting file directory and file name
            count+=1
        else:
            break
